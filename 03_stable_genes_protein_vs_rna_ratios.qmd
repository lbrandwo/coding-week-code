---
title: "03_stable_genes_protein_vs_rna_ratios.qmd"
author: Luke
format: html
---
# ------------------------------------------------------------------------------
...
```{r Load RNA Stable Genes and Protein Data}
# -------------------------------
...
```{r Calculating Protein - RNA Ratios (Log2FC differences)}
# -------------------------------
...
```{r Plotting Ratio Density Curves}
# -------------------------------
...
```{r Plots Investigating Variance Between Datasets and LogFC Distributions}
# -------------------------------
...
```{r Table for cellmine post}
# Create a formatted table from passed_genes_list
table_data <- lapply(names(passed_genes_list), function(contrast) {
  # Get the data for this contrast
  genes_df <- passed_genes_list[[contrast]]
  
  # Check if mgi_symbol is in columns or as rownames
  if (!"mgi_symbol" %in% colnames(genes_df)) {
    genes_df$mgi_symbol <- rownames(genes_df)
  }
  
  # Format each gene with its logFC_diff in brackets
  genes_formatted <- paste(genes_df$mgi_symbol, 
                          sprintf("(%.2f)", genes_df$logFC_diff), 
                          sep = " ")
  
  # Collapse all genes into one string
  genes_string <- paste(genes_formatted, collapse = ", ")
...
table_data$Contrast <- gsub("_vs_", "-", table_data$Contrast)

# Write to file (optional)
write.csv(table_data, "translation_on_demand/data/mm11/processed/passed_genes_table.csv", 
